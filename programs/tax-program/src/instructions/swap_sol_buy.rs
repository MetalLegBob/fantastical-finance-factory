...
use crate::constants::{
    amm_program_id, epoch_program_id, staking_program_id, treasury_pubkey, CARNAGE_SOL_VAULT_SEED,
...
use crate::helpers::faction_pool_reader::read_faction_pool;
use crate::helpers::pool_reader::native_mint;
...
pub fn handler<'info>(
    ...
    // Keep every nominal amount in the Tax/AMM pipeline amount-preserving.
    // This is normally trivial for WSOL and canonical faction mints, but
    // checking here keeps the policy fail-closed before Tax moves lamports.
    ...
    // Bind the economic identity to the authenticated AMM pool before any
    // calculation or value movement. SOL handlers are intentionally fixed to
    // AtoB/BtoA with WSOL in slot A, so reversed pools remain unsupported.
    let pool_view = read_faction_pool(&ctx.accounts.pool)?;
    require!(!pool_view.faction_is_a, TaxError::NonWsolQuotePool);
    require_keys_eq!(
        pool_view.quote_mint,
        native_mint(),
        TaxError::NonWsolQuotePool
    );
    require_keys_eq!(
    ...
    let faction_mint = if pool_view.is_crime {
        crate::constants::crime_mint()
    } else {
        crate::constants::fraud_mint()
    };
    require_keys_eq!(
    ...
    require!(
        ...
        TaxError::FactionWitnessMismatch
    );

    // The caller flag is now only an ABI witness. Pool-derived identity alone
    // selects the buy-tax field and every downstream accounting decision.
    let derived_is_crime = pool_view.is_crime;
    ...
    // =========================================================================
    // Pool identity, orientation, and mint accounts were bound above. Use the
    // same derived view for the floor so identity and economics cannot diverge.
    let sol_reserve = pool_view.quote_reserve;
    let token_reserve = pool_view.faction_reserve;
    ...
    // =========================================================================
    // 4-5. Split and execute native SOL tax distribution
    // =========================================================================
    let (staking_portion, carnage_portion, treasury_portion) = execute_split_distribution(
        &SplitDistributionAccounts {
            source: &ctx.accounts.user,
