...
pub fn epoch_program_id() -> Pubkey {
    Pubkey::from_str("64yriwHLm9JW8tkeKaiURmV7KdGACFSVs1psRrXjcGdY").unwrap()
}
...
pub fn amm_program_id() -> Pubkey {
    Pubkey::from_str("AG7QjhAoUecWK8iDDtzB4RzQY2HA7RgHbPeZimJSGwU1").unwrap()
}
...
pub fn staking_program_id() -> Pubkey {
    Pubkey::from_str("3wUWmbPUmsEzBmmCn9amftaLi1V7fai4bd3ANkyzT8HC").unwrap()
}
...
/// Seed for deriving the sweep_authority PDA.
/// This PDA owns the per-quote-asset sweep ATAs used by the SPL lanes.
pub const SWEEP_AUTHORITY_SEED: &[u8] = b"sweep_authority";

/// Derive the sweep authority PDA address.
///
/// # Returns
/// (pubkey, bump) tuple for the sweep authority PDA
pub fn get_sweep_authority_pda() -> (Pubkey, u8) {
    Pubkey::find_program_address(&[SWEEP_AUTHORITY_SEED], &crate::ID)
}
...
mod tests {
    ...
    fn test_amm_program_id() {
        ...
        assert_eq!(
            amm::ID,
            id,
            "Tax and its in-tree AMM dependency must compile for the same cluster"
        );
        #[cfg(not(feature = "devnet"))]
        assert_eq!(
            id.to_string(),
    ...
    #[test]
    fn test_sweep_authority_seed_and_pda() {
        assert_eq!(SWEEP_AUTHORITY_SEED, b"sweep_authority");
        assert_eq!(SWEEP_AUTHORITY_SEED.len(), 15);
        assert_eq!(
            get_sweep_authority_pda(),
            Pubkey::find_program_address(&[SWEEP_AUTHORITY_SEED], &crate::ID)
        );
    }
